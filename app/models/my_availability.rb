...
############# changed from here ##########
      i=0
      while i < params[:time_slots].count
      
        if params[:time_slots][i][:sno].present?
            already_slot = slots.select { |sl| 
            (sl["from"].to_time.strftime("%I:%M") == params[:time_slots][i][:start_time] || sl["to"].to_time.strftime("%I:%M") == params[:time_slots][i][:end_time])  } #&& sl[:sno] != params["sno"].to_s }
      ...
      # byebug
                if params.present? && slot["sno"] == params[:time_slots][i][:sno].to_s 
                    if slot[:from] == params[:time_slots][i][:start_time] && slot[:to] == params[:time_slots][i][:end_time]
                        return errors.add(:slot_error, 'You already have same slot for same time') 
                    else
                        slot["from"] =  (params[:time_slots][i][:start_time].to_time).strftime("%I:%M %p")
                        slot["to"] = (params[:time_slots][i][:end_time].to_time - 1.minute).strftime("%I:%M %p") # params[:time_slots][0][:end_time]
                    end
                end        
            end
            ...
            if params[:time_slots][i].present?
                start_sno = slots.empty? ? 1 :  slots.last["sno"].to_i + 1
                addl_slots = params["time_slots"][i]
                already_slot = slots.select { |sl|              
                (sl["from"].to_time.strftime("%I:%M") == params[:time_slots][i][:start_time].to_time.strftime("%I:%M") || sl["to"].to_time.strftime("%I:%M") == params[:time_slots][i][:end_time].to_time.strftime("%I:%M"))  }
                return errors.add(:slot_error, 'Cannot Add. You have different slot for same time')   if already_slot.present?

                s_time = params["time_slots"][i][:start_time]
                e_time = params["time_slots"][i][:end_time]
