...
# the following to be uncommented later
    ...
    def slots_list(params = nil)
      ...
      #    new_slot =   TimeSlotsCalculator.new.calculate_time_slots(
      #       slot["start_time"],  slot["end_time"], 59
      #       # slot["start_time"].to_time.strftime("%I:%M %p"),  slot["end_time"].to_time.strftime("%I:%M %p"), 59
      #       # params[:time_slots][:start_time], params[:time_slots][:end_time], 59
      #     )
      #   new_s << new_slot
      #   slots = (slots + new_s).flatten
      #   # slots.flatten
      #   end
      ...
      slots.each_with_index do |slot, index|
        ...
        if params.present? && slot[:sno] == params["sno"].to_s
          already_slot = slots.select{|sl| (sl[:from].to_time == params[:start_time].to_time or sl[:to].to_time == params[:end_time].to_time) && sl[:sno] != params["sno"].to_s }
          return errors.add(:slot_error, 'You have already slot for the day') if already_slot.present?
          slot[:from] = params["start_time"]
          slot[:to] = params["end_time"]
        end
