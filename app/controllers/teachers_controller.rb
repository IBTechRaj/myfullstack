  ...
  class TeachersController < ApplicationController
    ...
    def show
      ...
      if @teacher.blank?
        ...
        render json: TeacherSerializer.new(@teacher).serializable_hash, status: 200
      end
    end
    ...
    def activate_account
      ...
      @teacher = Teacher.find_by(id:@token.id).update(activated: true, uniq_id: set_uniq_id)
    ...
    def teacher_profile
     token = request.headers[:token] || params[:token]
      ...
      @teacher = Teacher.find_by(id: @token.id)
      if @teacher
        render json:  TeacherProfileSerializer.new(@teacher).serializable_hash, status: 200
      # else
        # render json: {errors: 'Invalid token'}, status: :unprocessable_entity
      end
    end
    ...
    def generate_token
      loop do
       ...
       uniq_id = "T#{number}"
       break uniq_id unless Teacher.where(uniq_id: uniq_id).exists?
      end
    end
